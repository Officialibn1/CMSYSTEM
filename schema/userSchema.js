...
const {
  AuthenticationError,
} = require("../config/errors/authenticationError.js");
...
const UserMutation = {
  ...
  eraseData: {
    ...
    async resolve(parent, { uid }, context) {
      ...
      try {
        const res1 = await Project.deleteMany({ userUID });

        const res2 = await Client.deleteMany({ userUID });
